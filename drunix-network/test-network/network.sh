...
# This script brings up the drunix network for testing smart contracts
# and applications. The test network consists of two organizations with one lite peer,
# one committing peer and one verification server node each, and a single node Raft ordering service.
#  Users can also use this script to create a channel deploy a chaincode on the channel
#
# Prerequisites:
#   Linux: git, Docker version <= 28, Golang, jq
#
...
function clearContainers() {
  ...
  ${CONTAINER_CLI} rm -f $(${CONTAINER_CLI} ps -aq --filter label=service=drunix) 2>/dev/null || true
...
# Versions known not to work with the test network
...
function checkPrereqs() {
  ## Check if peer binaries are available
  peer version > /dev/null 2>&1

  if [[ $? -ne 0 || ! -d "../config" ]]; then
    errorln "Peer binary and configuration files not found.."
    errorln "Check whether binary are generated. If not build the binary from {ROOTDIR} with (ex: make peer)"
    errorln
    # errorln "Follow the instructions in the Fabric docs to install the Fabric Binaries:"
    # errorln "https://hyperledger-fabric.readthedocs.io/en/latest/install.html"
    exit 1
  fi
  # use the peer container to see if the samples and binaries match your
  # docker images
  LOCAL_VERSION=$(peer version | sed -ne 's/^ Version: //p')
  ## change your peer image
  DOCKER_IMAGE_VERSION=$(${CONTAINER_CLI} run --rm npcioss/drunix-peer:1.0.0 peer version | sed -ne 's/^ Version: //p')
  ...
  for UNSUPPORTED_VERSION in $NONWORKING_VERSIONS; do
    ...
    if [ $? -eq 0 ]; then
      fatalln "Local binary version of $LOCAL_VERSION does not match the versions supported by the test network."
    fi

    infoln "$DOCKER_IMAGE_VERSION" | grep -q $UNSUPPORTED_VERSION
    if [ $? -eq 0 ]; then
      fatalln "Docker image version of $DOCKER_IMAGE_VERSION does not match the versions supported by the test network."
    fi
  done
...
function createPeerCerts() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-peer1.yaml --output="peer1"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
...
# 
  ...
  if [ "${DATABASE}" == "yugabyte" ]; then
  
    docker-compose -f scripts/yugabyte/compose.yaml up -d
    # # if ! ${CONTAINER_CLI} ps --filter "name=yb-tserver1-n1" --filter "status=running" --format '{{.Names}}' | grep -q '^yugabyte$'; then
    #   fatalln "Yugabyte container is not running. Please check logs."
    # else
    #   echo "Yugabyte container is up and running."
    # fi
    sleep 30
  fi
...
function createChannel() {
  ...
  CONTAINERS=($($CONTAINER_CLI ps | grep drunix- | awk '{print $2}'))
  len=$(echo ${#CONTAINERS[@]})
...
function networkDown() {
  ...
  if [ "${DATABASE}" == "yugabyte" ]; then
    # scripts/yugabyte/remove_yb.sh
    docker-compose -f scripts/yugabyte/compose.yaml down
  fi
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    clearContainers
    ${CONTAINER_CLI} volume rm compose_orderer.example.com compose_peer0.org1.example.com compose_peer1.org1.example.com compose_peer2.org1.example.com compose_peer0.org2.example.com compose_peer1.org2.example.com compose_peer2.org2.example.com
...
if [ "$MODE" == "prereq" ]; then
  infoln "Installing binaries and images. Fabric Version: ${IMAGETAG}  Fabric CA Version: ${CA_IMAGETAG}"
  installPrereqs
elif [ "$MODE" == "up" ]; then
