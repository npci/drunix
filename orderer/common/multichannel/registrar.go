...
import (
	...
	"github.com/npci/drunix/orderer/common/follower"
	...
	"github.com/npci/drunix/orderer/consensus/etcdraft"
...
type Registrar struct {
	...
	maxBatchSize         uint32
	registrarReleaseChan chan struct{}
}

/*
DRUNIX: this function will release the channel on sparseRegistrar intializer
*/
func (r *Registrar) ReleaseRegistrarChannel() {
	_, ok := <-r.registrarReleaseChan
	if !ok {
		return
	}
}
...
/*
DRUNIX: this function will notify the sparseRegistrar that a new channel is added
*/
func (r *Registrar) BlockRegistrarChannel() {
	r.registrarReleaseChan <- struct{}{}
}
...
func NewRegistrar(
	...
	r := &Registrar{
		...
		maxBatchSize:                config.General.Drunix.MaxBatchSize,
		registrarReleaseChan:        make(chan struct{}),
	}
...
func (r *Registrar) BroadcastChannelSupport(msg *cb.Envelope) (*cb.ChannelHeader, bool, *ChainSupport, error) {
	...
	if msg.LeanEnv != nil {
		...
		// && msg.Type == cb.HeaderType_ENDORSER_TRANSACTION {
		chdr.ChannelId = msg.LeanEnv.ChannelId
		/*
			DRUNIX: since new txn format has a different channelHeader struct and
			       endorser txn requires only channelId hence returning that.
				   for config txn the vanilla unmarshaller will be used
		*/
...
func (r *Registrar) newChain(configtx *cb.Envelope) {
	...
	// fixes https://github.com/npci/drunix/issues/2931
...
func (r *Registrar) JoinChannel(channelID string, configBlock *cb.Block, isAppChannel bool) (info types.ChannelInfo, err error) {
	...
	if configBlock.Header.Number == 0 && isMember {
		...
		if err == nil {
			...
			chain.start()
			/*
				DRUNIX: this channel will notify the sparseRegistrar that a new channel is added to the network
			*/
			go r.BlockRegistrarChannel()
		}
		return info, err
	}
