...
type Consenter interface {
	...
	BatchOrder(env *cb.Envelope, configSeq uint64) error

	// Configure accepts a reconfiguration or returns an error indicating the cause of failure
	// It ultimately passes through to the consensus.Chain interface
	Configure(config *cb.Envelope, configSeq uint64) error
	BatchConfigure(config *cb.Envelope, configSeq uint64) error
...
func (bh *Handler) ProcessMessage(msg *cb.Envelope, addr string) (resp *ab.BroadcastResponse) {
	...
	if !isConfig {
		...
		// TODO: check for txn format version and fail it is not same
		// we will not change anything in the txn here to preserve hash
		logger.Debugf("Txn format in lean : %v", msg.LeanEnv != nil)
		// if msg.LeanEnv == nil {
		// 	fmt.Println("^^^^^^^^ processing vanilla format txn")
		// } else {
		// 	fmt.Println("^^^^^^^^ processing Lean format txn")
		// }
		...
		if bh.FollowerBatching {
			err = processor.BatchOrder(msg, configSeq)
		} else {
		...
		if bh.FollowerBatching {
			err = processor.BatchConfigure(config, configSeq)
		} else {
