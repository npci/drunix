...
import (
	...
	orderer_types "github.com/npci/drunix/orderer/common/types"
	"github.com/npci/drunix/orderer/consensus/etcdraft"
	"github.com/npci/drunix/orderer/consensus/etcdraft/mocks"
	consensusmocks "github.com/npci/drunix/orderer/consensus/mocks"
	mockblockcutter "github.com/npci/drunix/orderer/mocks/common/blockcutter"
...
var _ = Describe("Chain", func() {
	...
	Describe("Single Raft node", func() {
		...
		JustBeforeEach(func() {
			rpc := &mocks.FakeRPC{}
			chain, err = etcdraft.NewChain(support, opts, configurator, rpc, cryptoProvider, noOpBlockPuller, fakeHaltCallbacker.HaltCallback, observeC, 2.0, false)
			Expect(err).NotTo(HaveOccurred())
		...
		Context("when a node starts up", func() {
			It("WAL 32MB file size should be within size limits", func() {
				...
				_, err := etcdraft.NewChain(support, opts, configurator, nil, cryptoProvider, noOpBlockPuller, nil, observeC, 2.0, false)
				Expect(err).NotTo(HaveOccurred())
			})
				...
				Describe("when a chain is started with existing WAL", func() {
					...
					Context("WAL file is not readable", func() {
						It("fails to load wal", func() {
							...
							c, err := etcdraft.NewChain(support, opts, configurator, nil, cryptoProvider, noOpBlockPuller, nil, observeC, 2.0, false)
							Expect(c).To(BeNil())
