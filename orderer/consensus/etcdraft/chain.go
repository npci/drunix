...
import (
	...
	"github.com/npci/drunix/orderer/common/cluster"
...
type Chain struct {
	...
	submitC  chan *submit
	batchC   chan *orderer.SubmitRequest
	applyC   chan apply
	...
	// Drunix configs
	BlockCutTime     float64
	followerBatching bool
}
...
func NewChain(
	...
	c := &Chain{
		...
		batchC:            make(chan *orderer.SubmitRequest),
		...
		BlockCutTime:     blockCutTime,
		followerBatching: followerBatching,
	}
...
/*
DRUNIX: This will create batches if it's a follower too to utilize follower batching
*/
func (c *Chain) BatchOrder(env *common.Envelope, configSeq uint64) error {
	c.Metrics.NormalProposalsReceived.Add(1)
	c.logger.Debug("Received a non config transaction - submitting it to batcher")
	c.batchC <- &orderer.SubmitRequest{
		Channel:           c.channelID,
		LastValidationSeq: configSeq,
		Payload:           env,
		IsConfig:          false,
		Type:              orderer.SubmitRequestType_ENVELOPE,
	}
...
// BatchConfigure for config flow
func (c *Chain) BatchConfigure(env *common.Envelope, configSeq uint64) error {
	c.Metrics.ConfigProposalsReceived.Add(1)
	c.logger.Info("Received a config transaction - submitting it to batcher")
	c.batchC <- &orderer.SubmitRequest{
		Channel:           c.channelID,
		LastValidationSeq: configSeq,
		Payload:           env,
...
func (c *Chain) run() {
	...
	startTimer := func() {
		if !ticking {
			...
			timer.Reset(time.Duration(c.BlockCutTime * float64(time.Second)))
		}
	}
...
func (c *Chain) runBatch() {
	...
	for {
		...
		case s := <-submitC:
			...
			// c.propose(propC, bc, batches...)
			c.proposeBatch(propC, bc, s.req.Batches...)
...
func (c *Chain) orderedBatch(msg *orderer.SubmitRequest) (batches []*orderer.Batch, pending bool, err error) {
	...
	if msg.IsConfig {
		...
		batches := []*orderer.Batch{}
		existingPendingBatch := c.support.BlockCutter().CutBatch()

		if len(existingPendingBatch.Reqs) != 0 {
			batches = append(batches, existingPendingBatch)
		}
		configBatch := &orderer.Batch{
			Reqs: []*orderer.SubmitRequest{},
		}

		configBatch.Reqs = append(configBatch.Reqs, msg)

		batches = append(batches, configBatch)
		return batches, false, nil
	}

	batches, pending = c.support.BlockCutter().OrderedBatch(msg)
	c.logger.Debugf("Batches: %v, Pending: %v", batches == nil, pending)
	return batches, pending, nil
}
...
func (c *Chain) isConfig(env *common.Envelope) (bool, error) {
	if env.Type == common.HeaderType_ENDORSER_TRANSACTION ||
		env.Type == common.HeaderType_APPROVE || env.Type == common.HeaderType_COMMIT {
		return false, nil
	} else {
...
/*
DRUNIX:
batcher creates batches and submits them to the orderer through the submit function
Create batches in two ways: size and timer
*/
func (c *Chain) batcher(batchDoneC chan struct{}) {
	ticking := false
	timer := c.clock.NewTimer(time.Second)
	// we need a stopped timer rather than nil,
	// because we will be select waiting on timer.C()
	...
	startTimer := func() {
		if !ticking {
			ticking = true
			// timer.Reset(c.support.SharedConfig().BatchTimeout(c.BlockCutTime)) // here
			timer.Reset(time.Duration(c.BlockCutTime * float64(time.Second)))
		}
	}
	...
	for {
		...
		/*
			DRUNIX:
			Retrieve requests from batchC channel
			req is retrieved from batchC (chan *orderer.SubmitRequest)
			In vanilla, req consists only of an envelope, whereas here, instead of a single envelope,
			batches of envelopes are received through from SubmitRequest
			The req is ordered to create batches, which are then submitted to the orderer
		*/
		case req := <-c.batchC:
			// fmt.Printf("-------reached batcher %v %v\n", req.Type, len(req.Payload.Payload))
			...
			if pending {
				c.logger.Debugf("Starting timer ")
				startTimer() // no-op if timer is already started
			} else {
				c.logger.Infof("Stopping timer ")
				stopTimer()
			}

			// batchesToSubmit := make([]*orderer.Batch, 0)
			// for _, batch := range batches {
			// 	batchesToSubmit = append(batchesToSubmit, &orderer.Batch{Batch: batch})
			...
			if len(batches) > 0 && len(batches[0].Reqs) > 0 {
				...
				if err != nil {
					c.logger.Warningf("Failed to submit batches")
				}
			}
		case <-timer.C():
			ticking = false

			c.logger.Debug("Cutting with time ticking...... ")
			batch := c.support.BlockCutter().CutBatch()
			...
			// else {
			// 	// c.logger.Info("Found the batch, submitting it to Submit() ", batch.Reqs[0].Type, len(batch.Reqs[0].Payload.Payload))
			// }

			batchesToSubmit := make([]*orderer.Batch, 0)
			batchesToSubmit = append(batchesToSubmit, batch)
			...
			if err != nil {
				c.logger.Warningf("Failed to submit batches when the timer intiatied it")
			}
		case <-batchDoneC:
			stopTimer()
			return
...
/*
DRUNIX:checkValidationStatusOfTxns checks the validation status of transactions in
...
func (c *Chain) checkValidationStatusOfTxns(inbatch *orderer.Batch) (*orderer.Batch, error) {
	...
	if inbatch.Reqs[0].IsConfig {
		// ConfigMsg
		if inbatch.Reqs[0].LastValidationSeq < seq {
			c.logger.Errorf("Config message was validated against %d, although current config seq has advanced (%d)", inbatch.Reqs[0].LastValidationSeq, seq)
			inbatch.Reqs[0].Payload, _, err = c.support.ProcessConfigMsg(inbatch.Reqs[0].Payload)
			if err != nil {
				c.Metrics.ProposalFailures.Add(1)
	...
	outReq := make([]*orderer.SubmitRequest, 0)
	for _, req := range inbatch.Reqs {
		// it is a normal message
		if req.LastValidationSeq < seq {
...
/*
DRUNIX: This function will propose the batch from follower to the reader instead of single envelope
*/
func (c *Chain) proposeBatch(ch chan<- *common.Block, bc *blockCreator, batches ...*orderer.Batch) {
	// fmt.Printf("length of batches : %v\n", len(batches))
	for _, batch := range batches {
		c.logger.Info("The incoming BATCH has :", len(batch.Reqs))
		batch, err := c.checkValidationStatusOfTxns(batch)
		// c.logger.Debug("The incoming BATCH has ", len(batch.Reqs))
		if err != nil {
			c.logger.Error("Failed to validate the batch. Ignoring the txn, must be a config txn")
			return
		}
		b := bc.createNextBlockWithBatch(batch)
		c.logger.Infof("Created block [%d], there are %d blocks in flight", b.Header.Number, c.blockInflight)
