...
func (c *Consenter) HandleChain(support consensus.ConsenterSupport, metadata *common.Metadata) (consensus.Chain, error) {
	...
	var snapShotSize uint32
	snapShotSize = m.Options.SnapshotIntervalSize
	if os.Getenv("ORDERER_SNAPSHOTINTERVAL") != "" {
		size, err := strconv.Atoi(os.Getenv("ORDERER_SNAPSHOTINTERVAL"))
		snapShotSize = uint32(size)
		if err != nil {
			snapShotSize = m.Options.SnapshotIntervalSize
		}
	}
	...
	opts := Options{
		...
		SnapshotIntervalSize: snapShotSize,
	...
	return NewChain(
		...
		haltCallback,
		nil,
		c.OrdererConfig.General.Drunix.BlockCutTime,
		c.OrdererConfig.General.Drunix.FollowerBatching,
	)
}
