...
import (
	...
	"github.com/npci/drunix/common/chaincode"
...
func (c *Cache) Initialize(channelID string, qe ledger.SimpleQueryExecutor) error {
	...
	/*
		DRUNIX
		During the initial chaincode container setup, existing containers are not removed.
		However, lite peers do not process blocks, so they are re-initialized on chaincode updates.
		To handle this, set 'initialize' to false so that after each initialization, any old containers are removed if present.
	*/
	return c.update(false, channelID, dirtyChaincodes, qe)
}
