...
func (s *Store) Persist(txid string, blockHeight uint64,
	...
	// DRUNIX : if KVStore is enabled redirect
	if s.KVStore != nil {
		err := s.KVStore.Persist(txid, blockHeight, privateSimulationResultsWithConfig)
		if err != nil {
			logger.Errorf("Persisting private data to KVtransient store for txid [%s] at block height [%d] failed with err:%+v", txid, blockHeight, err)
		}
	}
