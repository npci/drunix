...
func (p *baseEvaluator) EvaluateLtx(blockNum, txNum uint64, NsRwSets []*common.NsReadWriteSet, ns string, sd []*protoutil.SignedData) commonerrors.TxValidationError {
	// iterate over all writes in the rwset
	for _, nsRWSet := range NsRwSets {
		// skip other namespaces
		if nsRWSet.Namespace != ns {
...
type RWSetPolicyEvaluator interface {
	...
	EvaluateLtx(blockNum, txNum uint64, NsRwSets []*common.NsReadWriteSet, ns string, sd []*protoutil.SignedData) commonerrors.TxValidationError
}
...
// Validate implements the function of the StateBasedValidator interface
func (klv *KeyLevelValidator) ValidateLtx(cc string,
	...
	for _, endorsement := range endorsements {
		...
		signedData, err := protoutil.EndorsementAsSignedDataForLightEnv(endorsement, prpBytes)
		...
		// pEnd := &msp.SerializedIdentity{}
		// err = proto.Unmarshal(endBytes, pEnd)
		// if err != nil {
		// 	return policyErr(errors.WithMessagef(err, "common.Endorsement failed Marshalling (%d,%d)", blockNum, txNum))
		// }

		// pEndBytes, err := proto.Marshal(pEnd)

		// logger.Infof("prpBytes: %x", prpBytes)
		// logger.Infof("endBytes: %s", endBytes)

		// data := make([]byte, len(prpBytes)+len(endBytes))
		// copy(data, prpBytes)
		// copy(data[len(prpBytes):], endBytes)

		// logger.Infof("data: %x", data)
		// logger.Infof("pendBytes: %x", pEndBytes)
		// logger.Infof("Signature: %x", endorsement.Signature)

		// datahash := sha256.New()
		// datahash.Write(data)
		// signhash := sha256.New()
		// signhash.Write(endorsement.Signature)
		// endhash := sha256.New()
		// endhash.Write(endBytes)
		// fmt.Printf("\nVAL-data, endbytes, sign:: %x, %x, %x\n", datahash.Sum(nil), endhash.Sum(nil), signhash.Sum(nil))
		...
		// datahash := sha256.New()
		// datahash.Write(signedData[0].Data)
		// signhash := sha256.New()
		// signhash.Write(signedData[0].Signature)
		// endhash := sha256.New()
		// endhash.Write(signedData[0].Identity)
		// fmt.Printf("\nVAL-data, endbytes, sign:: %x, %x, %x\n", datahash.Sum(nil), endhash.Sum(nil), signhash.Sum(nil))
	...
	// return the decision of the policy evaluator
	err1 := policyEvaluator.EvaluateLtx(blockNum, txNum, rwset.NsRwset, cc, signatureSet)
	if err1 != nil {
		// If endorsement policy check fails, log the endorsement policy and endorser identities.
		// No need to handle Unmarshal() errors since it will simply result in endorsementPolicy being empty in the log message.
		endorsementPolicy := &peer.ApplicationPolicy{}
