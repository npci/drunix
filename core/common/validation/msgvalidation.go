...
func ValidateLtxTransaction(e *common.Envelope, cryptoProvider bccsp.BCCSP) pb.TxValidationCode {
	putilsLogger.Debugf("ValidateTransactionEnvelope starts for envelope %p", e)
	...
	chdr, shdr, err := validateLtxCommonHeader(e)
	if err != nil {
		putilsLogger.Errorf("validateCommonHeader returns err %s", err)
		return pb.TxValidationCode_BAD_COMMON_HEADER
	}
	// var channelId string
	var envType common.HeaderType
	if chdr == nil {
		// channelId = e.LeanEnv.ChannelId
		envType = e.Type
	} else {
		// channelId = chdr.ChannelId
		envType = common.HeaderType(chdr.Type)
	}

	/* DRUNIX:
	vanilla : client signs the proposal and send it to endorsing peer for endorsement.
	Drunix: client signs vanilla proposal and sneds it to lite-peer for endorsement but in LP the vanilla envelope is change to lite envelope. This causes the signature mismatch during signing and verification. Hence commented the verification part.
	TODO:
	 Either the client should sign the lite envelope or the verification should happen on vanilla envelope.
	*/

	/*
		// DRUNIX: for endorser txn the identity and signature data are in lean format. Construct signed data to retrieve this data.
		signedData, err := protoutil.EnvelopeAsSignedData(e)
		if err != nil {
			putilsLogger.Errorf("EnvelopeAsSignedData returns err %s", err)
			return pb.TxValidationCode_BAD_CREATOR_SIGNATURE
		}

		err = checkSignatureFromCreator(signedData[0].Identity, signedData[0].Signature, signedData[0].Data, channelId, cryptoProvider)
		if err != nil {
			putilsLogger.Errorf("checkSignatureFromCreator returns err %s", err)
			return pb.TxValidationCode_BAD_CREATOR_SIGNATURE
		}
	*/
	...
	case common.HeaderType_ENDORSER_TRANSACTION:
		...
		/*
			DRUNIX:
				- proposal hash verification happens only for chaincode approve and chaincode commit txns
				- in endorser txns apart from cc approve/commit we are not passing the payload in envelope so it's difficult to verify hash
		*/
		if e.Payload != nil {
			payload, err := protoutil.UnmarshalPayload(e.Payload)
			if err != nil {
...
func validateLtxCommonHeader(env *common.Envelope) (*common.ChannelHeader, *common.SignatureHeader, error) {
	...
	if env.Type == common.HeaderType_ENDORSER_TRANSACTION {
		// DRUNIX: Meta is not required for phase II
		if env.LeanEnv.Meta != nil {
			// 	/*DRUNIX:
			// 	- For lean format we do not have channel header, so no validation is done.
			// 	- The required fields of channel header are included in the lean format
			// 	- In vanilla fabric, there is not much happening in channel header validation, they are checking the type of the transaction.
			// 	*/
			err := validateSignatureHeader(env.LeanEnv.Meta.SignatureHeader)
			if err != nil {
				return nil, nil, err
			}
			return nil, env.LeanEnv.Meta.SignatureHeader, nil
		}
		return nil, nil, nil
	}
