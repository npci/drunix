...
import (
	...
	"github.com/npci/drunix/common/viperutil"
	"github.com/npci/drunix/core/config"
	"github.com/npci/drunix/internal/pkg/comm"
	gatewayconfig "github.com/npci/drunix/internal/pkg/gateway/config"
...
type Config struct {
	...
	/*
		DRUNIX:
		CommittingPeerConfig holds the configuration details required to establish and
		maintain a gRPC connection with the committing peer. This connection is essential
		for retrieving the commit status of transactions after they are submitted to the network.
	*/
...
func (c *Config) load() error {
	...
	c.LitePeerEnabled = viper.GetBool("peer.litepeer.enabled")

	/*
		DRUNIX
		Initialize CommittingPeerConfig by loading configuration values from environment
		variables using Viper. This ensures the committing peer connection details are
		dynamically sourced from the application's runtime configuration.
	*/
	c.CommittingPeerConfig = GetCommittingPeerConfig()
...
dynamically sourced from the application's runtime configuration.
*/
type CommittingPeerConfig struct {
	PeerId                     string
	PeerEndpoint               string
	TlsCertPath                string
	ForwardCommitStatusEnabled bool
	// DRUNIX:- InitialRetryDelay defines the base wait time before the first retry attempt after a failure. Default: 200ms
	InitialRetryDelay time.Duration
	// DRUNIX:- MaxRetryDelay specifies the upper bound for exponential backoff; retry delays will not exceed this value. Default: 30s
...
func GetCommittingPeerConfig() *CommittingPeerConfig {
	...
	if initialRetryDelay == time.Duration(0) {
		initialRetryDelay = 200 * time.Millisecond
	}

	if maxRetryDelay == time.Duration(0) {
		maxRetryDelay = 30 * time.Second
	}

	return &CommittingPeerConfig{
		PeerId:                     viper.GetString("peer.committingpeer.id"),
		PeerEndpoint:               viper.GetString("peer.committingpeer.endpoint"),
		TlsCertPath:                viper.GetString("peer.committingpeer.tlscertpath"),
		ForwardCommitStatusEnabled: viper.GetBool("peer.committingpeer.forwardcommitstatusrequests.enabled"),
