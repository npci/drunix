...
import (
	...
	commonledger "github.com/npci/drunix/common/ledger"
	"github.com/npci/drunix/common/policies"
	"github.com/npci/drunix/common/semaphore"
	"github.com/npci/drunix/core/committer"
	"github.com/npci/drunix/core/committer/txvalidator"
	"github.com/npci/drunix/core/committer/txvalidator/plugin"
	validatorv14 "github.com/npci/drunix/core/committer/txvalidator/v14"
	txnVal "github.com/npci/drunix/core/committer/txvalidator/v20"
	validatorv20 "github.com/npci/drunix/core/committer/txvalidator/v20"
	...
	validation "github.com/npci/drunix/core/handlers/validation/api/state"
...
func (d DeliverChainManager) GetChain(chainID string, _ string) deliver.Chain {
...
type Peer struct {
	...
	pluginMapperAdapter plugin.MapperAdapter

	PeerId               string
	CommittingPeerConfig *CommittingPeerConfig
}
...
func (p *Peer) openStore(cid string) (*transientstore.Store, error) {
	...
	if p.KVStoreProvider != nil {
		kvstore, err := p.KVStoreProvider.OpenStore(cid)
		if err != nil {
			return nil, err
		}
		store.KVStore = kvstore.KVStore
	}

	store.LitePeerEnabled = p.LitePeerEnabled
...
func (p *Peer) CreateChannel(
	...
	/*
		DRUNIX
		Added a configuration update listener for lite peers during channel creation.
	*/
	if p.LitePeerEnabled {
		if err := p.createChannelOnConfigUpdate(cid, l, deployedCCInfoProvider, legacyLifecycleValidation, newLifecycleValidation); err != nil {
			return err
		}
	} else {
...
func (p *Peer) createChannel(
	...
	channel := &Channel{
		...
		chaincodeEventPublisher: p.ChaincodeEventPublisher,
	}
	...
	var txV20Validator txvalidator.Validator

	vsccServiceConfig := vscc_service.LoadConfig()
	if vsccServiceConfig.Enabled {
		vsccServiceClient, err := vscc_service.NewVsccServiceClient(vsccServiceConfig)
		if err != nil {
			logger.Errorf("[channel %s] failed connecting to vscc service : %v", bundle.ConfigtxValidator().ChannelID(), err)
			return errors.Wrapf(err, "[channel %s] failed connecting to vscc service", bundle.ConfigtxValidator().ChannelID())
		}
		txV20Validator = validatorv20.NewTxValidatorPeerAdapter(
			cid,
		...
		txV20Validator = validatorv20.NewTxValidator(
			cid,
			p.validationWorkersSemaphore,
			channel,
			channel.Ledger(),
	...
	validator := &txvalidator.ValidationRouter{
		...
		V20Validator: txV20Validator,
	}

	txnVal.IsLtfEnabled.Store(validator.Capabilities().LeanFormatEnabled())
	logger.Warningf("Value for IsLtfEnabled has been reset to  %v\n", txnVal.IsLtfEnabled.Load())
	...
	// Initialize the channel's chaincode events publisher
	if publisher.LoadConfig().ServerType != publisher.UNIMPLEMENTED {
		if err := channel.InitPublishChaincodeEvents(); err != nil {
			peerLogger.Errorf("Error initializing publishing chaincode events for channel %s: %s", cid, err)
			return err
		}
	}
	...
	logger.Info("created channel : ", cid)
...
func (p *Peer) Initialize(
	...
	newLifecycleValidation plugindispatcher.CollectionAndLifecycleResources,
	nWorkers int,
	pmAdapter plugin.MapBasedMapperAdapter,
) {
	...
	p.pluginMapperAdapter = pmAdapter
	...
	for _, cid := range ledgerIds {
		...
		*/
		if p.LitePeerEnabled {
			err = p.createChannelOnConfigUpdate(cid, nil, deployedCCInfoProvider, legacyLifecycleValidation, newLifecycleValidation)
			if err != nil {
