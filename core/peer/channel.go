...
import (
	...
	"github.com/npci/drunix/common/ledger/blockledger/fileledger"
...
type Channel struct {
	...
	// chaincodeEventPublisher is used to publish chaincode events to the event hub
	chaincodeEventPublisher publisher.Publisher
	LeanEnabled             bool
}

// InitPublishChaincodeEvents spins up a routine to publishes chaincode events to the event hub.
func (c *Channel) InitPublishChaincodeEvents() error {
	// fetch current blockchain info
	blockchainInfo, err := c.ledger.GetBlockchainInfo()
	if err != nil {
		return err
	}

	// get block iterator from recent block height
	ledgerIter, err := c.ledger.GetBlocksIterator(blockchainInfo.GetHeight())
	if err != nil {
		return err
	}

	// loop for each block's events and publish in a dedicated routine
	go func() {
		eventsIter := event.NewChaincodeEventsIterator(ledgerIter)
		defer eventsIter.Close()
		// TODO stop the loop if peer exits from channel
		for {
			response, err := eventsIter.Next()
			if err != nil {
				peerLogger.Errorw("Error getting next chaincode events", "channel", c.id, "error", err)
				// not returning error here because we don't want to stop the event publishing loop
				continue
			}
			if err := c.chaincodeEventPublisher.Publish(c.id, response); err != nil {
				peerLogger.Errorw("Error publishing chaincode events", "channel", c.id, "error", err)
				// not returning error here because we don't want to stop the event publishing loop
				continue
			}
		}
