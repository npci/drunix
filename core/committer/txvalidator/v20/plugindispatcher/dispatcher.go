...
func (v *dispatcherImplAdapter) GetInfoForValidateLtx(channelId string, ccID string) (string, []byte, error) {
	// obtain name of the validation plugin and the policy
	plugin, args, err := v.getCDataForCC(channelId, ccID)
	if err != nil {
		msg := fmt.Sprintf("Unable to get chaincode data from ledger for ccId %s, due to %s", ccID, err)
		logger.Errorf(msg)
		return "", nil, err
	}
	return plugin, args, nil
...
func (v *dispatcherImplAdapter) txWritesToNamespaceLtx(ns *common.NsReadWriteSet) bool {
	// check for public writes first
	if ns.Rwset != nil && len(ns.Rwset.Writes) > 0 {
		return true
	}
	...
	if ns.Rwset != nil && len(ns.Rwset.MetadataWrites) > 0 {
		return true
	}
