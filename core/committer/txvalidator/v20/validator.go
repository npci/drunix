...
func NewTxValidator(
	...
	cryptoProvider bccsp.BCCSP,
	pmAdapter plugin.MapperAdapter,
) *TxValidator {
	...
	pluginValidatorAdapter := plugindispatcher.NewPluginValidatorAdapter(pmAdapter, ler, &dynamicDeserializer{cr: cr}, &dynamicCapabilities{cr: cr}, channelPolicyManagerGetter, cor)
	...
	return &TxValidator{
		...
		DispatcherAdapter: plugindispatcher.NewAdapter(channelID, cr, ler, lcr, pluginValidatorAdapter),
	}
}
		...
		for tIdx, d := range block.Data.Data {
			...
			go func(index int, data []byte) {
				...
				if v.ChannelResources.Capabilities().LeanFormatEnabled() {
					v.validateLtxTx(&blockValidationRequest{
						d:     data,
						block: block,
						tIdx:  index,
					}, results)
...
func (v *TxValidator) validateTx(req *blockValidationRequest, results chan<- *blockValidationResult) {
	...
	if env, err := protoutil.GetEnvelopeFromBlock(d); err != nil {
		...
		if common.HeaderType(chdr.Type) == common.HeaderType_ENDORSER_TRANSACTION {
			...
			IsLtfEnabled.Store(v.ChannelResources.Capabilities().LeanFormatEnabled())
			logger.Warning("Value for IsLtfEnabled has been updated as ", IsLtfEnabled.Load())
