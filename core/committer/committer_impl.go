...
type PeerLedgerSupport interface {
	...
	// DRUNIX: commit flow for lite transaction format
	CommitLegacyLtx(blockAndPvtdata *ledger.BlockAndPvtData, commitOpts *ledger.CommitOptions) error
...
// DRUNIX: commit flow for lite transaction format
func (lc *LedgerCommitter) CommitLegacyLtx(blockAndPvtData *ledger.BlockAndPvtData, commitOpts *ledger.CommitOptions) error {
	// Committing new block
	if err := lc.PeerLedgerSupport.CommitLegacyLtx(blockAndPvtData, commitOpts); err != nil {
		return err
	}
