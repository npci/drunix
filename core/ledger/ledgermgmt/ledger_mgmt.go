...
func NewLedgerMgr(initializer *Initializer) *LedgerMgr {
	...
	ledgerMgr := &LedgerMgr{
		...
		litePeerEnabled:      initializer.LitePeerEnabled,
	}
...
func (m *LedgerMgr) OpenLedger(id string) (ledger.PeerLedger, error) {
	...
	/*
		DRUNIX
		When config events are received from the committing peer, lite peers are re-initialized.
		During re-initialization, ledgers are reopened, so the ledger-open check for lite peers has been removed.
	*/
	if !m.litePeerEnabled {
