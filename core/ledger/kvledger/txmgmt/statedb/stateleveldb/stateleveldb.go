...
func (provider *VersionedDBProvider) GetKVHandle(dbName string) (statedb.KVersionStoreHandler, error) {
	return newVersionedDB(provider.dbProvider.GetDBHandle(dbName), dbName), nil
}
...
// GetVersion implements method in VersionedDB interface
func (vdb *versionedDB) GetKeyVersion(namespace string, key string) (*version.Height, error) {
	versionedValue, err := vdb.GetMVCCState(namespace, key)
	if err != nil {
		return nil, err
	}
	if versionedValue == nil {
		return nil, errors.Errorf("key: %v not found in db", key)
	}
	logger.Debugf("versionedValue.Version: %v with error: %v\n", versionedValue.Version, err)
	return versionedValue.Version, nil
}

func (vdb *versionedDB) GetMVCCState(namespace string, key string) (*statedb.VersionMVCC, error) {
	logger.Debugf("GetState(). ns=%s, key=%s", namespace, key)
	dbVal, err := vdb.db.Get(encodeDataKey(namespace, key))
	if err != nil {
		return nil, err
...
/*
DRUNIX: This method is required to use orderer MVCC updates
*/
func (vdb *versionedDB) ApplyOrdererUpdates(updates map[string]*statedb.VersionMVCC) error {
	dbBatch := vdb.db.NewUpdateBatch()
	for k, vv := range updates {
		dataKey := []byte(k)
		// logger.Warnf("Channel [%s]: Applying key(string)=[%s] key(bytes)=[%#v]", vdb.dbName, string(dataKey), dataKey)
		if vv.Is_Delete {
			dbBatch.Delete(dataKey)
		} else {
			encodedVal, err := encodeValueMVCC(vv)
			if err != nil {
				return err
			}
			dbBatch.Put(dataKey, encodedVal)
