...
func NewKeyVersion(blockNumber uint64, txIndx uint64) *version.Height {
	return version.NewHeight(blockNumber, txIndx)
}
...
func NewLtxVersion(protoVersion *common.Version) *version.Height {
	if protoVersion == nil {
		return nil
	}
	return version.NewHeight(protoVersion.BlockNum, protoVersion.TxNum)
...
func NewLiteVersion(protoVersion *cb.Version) *version.Height {
	if protoVersion == nil {
		return nil
	}
	return version.NewHeight(protoVersion.BlockNum, protoVersion.TxNum)
...
func IsLtxKVWriteDelete(kvWrite *common.KVWrite) bool {
	return kvWrite.IsDelete || len(kvWrite.Value) == 0
}
...
func IsLtxKVWriteHashDelete(kvWriteHash *common.KVWriteHash) bool {
	return kvWriteHash.IsDelete || len(kvWriteHash.ValueHash) == 0 || bytes.Equal(hashOfZeroLengthByteArray, kvWriteHash.ValueHash)
}
