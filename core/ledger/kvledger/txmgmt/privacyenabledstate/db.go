...
func NewDBProvider(
	...
	if stateDBConf != nil && stateDBConf.StateDatabase == ledger.CouchDB {
		...
		// DRUNIX:
		// Added SQLDB implementation of versionedDBProvider interface
	} else if stateDBConf != nil && stateDBConf.StateDatabase == ledger.SqlDB {
		if vdbProvider, err = statesqldb.NewVersionedDBProvider(stateDBConf.SqlDB, metricsProvider, sysNamespaces); err != nil {
			return nil, err
		}
	} else {
