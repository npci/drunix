...
type Config struct {
	...
	// VsccServerConfig holds the configuration parameters for the vscc service
	// VsccConfig *VsccConfig
}
...
type StateDBConfig struct {
	...
	// DRUNIX:
	// sqldb is the configuration for SQLDB.  It is used when StateDatabase
	// is set to "sqldb".
	SqlDB *SqlDbConfig
}
...
// type VsccConfig struct {
// // ConfigTxChannelName is the name of the channel which vscc service will listen for config updates
// ConfigTxChannelName string
// // ConfigTxChannelTimeout is the timeout duration of the subcriber which listens for config updates
// ConfigTxChannelTimeout time.Duration
// ValidatorPoolSize is the number of validator that can be spawned concurrently
...
type PeerLedger interface {
	...
	CommitLegacyLtx(blockAndPvtdata *BlockAndPvtData, commitOpts *CommitOptions) error
