...
func (d *deliverServiceImpl) StartDeliverForChannel(chainID string, ledgerInfo blocksprovider.LedgerInfo, finalizer func()) error {
	...
	dc := &blocksprovider.Deliverer{
		...
		LitePeerEnabled:     false,
	}
	/*
		DRUNIX:
			orderer maintains a seperate chain for lite peer. To let orderer know that a particular peer is lite peer we added an env `CORE_PEER_LITE_ENABLED`.
	*/
	if d.conf.DeliverServiceConfig.LitePeerEnabled {
		dc.LitePeerEnabled = true
	}
