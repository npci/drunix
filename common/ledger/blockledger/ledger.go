...
DRUNIX:
*/
type SparseMetadataReadWriter interface {
	AppendOrgMerkleInfo(key []byte, orgVal []byte) error
	AppendOrgBlockIndex(key []byte, orgVal []byte) error
	SaveChannelHeadInfo(key []byte, orgVal []byte) error
