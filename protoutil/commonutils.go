...
func IsLiteBlock(block *cb.Block) bool {
	if block.Data == nil {
		return false
	}
...
func ChannelHeader(env *cb.Envelope) (*cb.ChannelHeader, error) {
	...
	/*
		DRUNIX: to avoid nil-pointer we should have a check here,
		since there is no concept of channelHeader in ltx-format
	*/
	if env.LeanEnv != nil && env.Type == cb.HeaderType_ENDORSER_TRANSACTION {
		return nil, nil
	}
