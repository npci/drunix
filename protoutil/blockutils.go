...
func GetChannelIDFromBlock(block *cb.Block) (string, error) {
	...
	if envelope.LeanEnv != nil {
		return envelope.LeanEnv.ChannelId, nil
	}
...
func InitBlockMetadata(block *cb.Block) {
	if block.Metadata == nil {
		...
		/*
			DRUNIX: since we added 4 more metadata in the block so we need to initialize it accordingly
		*/
		block.Metadata = &cb.BlockMetadata{Metadata: [][]byte{{}, {}, {}, {}, {}, {}, {}, {}, {}}}
	} else if len(block.Metadata.Metadata) < int(cb.BlockMetadataIndex_SPARSE_TXN_FILTER+1) {
		...
		for i := int(len(block.Metadata.Metadata)); i <= int(cb.BlockMetadataIndex_SPARSE_TXN_FILTER); i++ {
...
func VerifyTransactionsAreWellFormed(block *cb.Block) error {
	...
	// DRUNIX-LTF-TODO : if lean format verfify transactions
