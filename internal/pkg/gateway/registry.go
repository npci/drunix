...
import (
	...
	gossipapi "github.com/npci/drunix/gossip/api"
	gossipcommon "github.com/npci/drunix/gossip/common"
	gossipdiscovery "github.com/npci/drunix/gossip/discovery"
...
func (reg *registry) endorsersByOrg(channel string, chaincode string) map[string][]*endorserState {
	...
	for _, es := range endorsersByOrg {
		...
		endorserOrg := ""
		endorserLogAddresses := make([]string, len(es))
		for index, e := range es {
			endorserOrg = e.endorser.mspid
			endorserLogAddresses[index] = e.endorser.logAddress
		}
		reg.logger.Infof("Org: %v, Endorsers: [%v]", endorserOrg, strings.Join(endorserLogAddresses, ","))
	}
...
func (reg *registry) channelMembers(channel string) gossipdiscovery.Members {
	...
	/*
		DRUNIX:-
		Disabled using local ledger height, as lite peers must rely on the committing
		peer’s propagated height. This function is used only during endorsement, so no
		lite/committing peer distinction is required here.
	*/
...
func sorter(e []*endorserState, host string) func(i, j int) bool {
	return func(i, j int) bool {
		if e[i].height == e[j].height {
			/*
				DRUNIX:-
				Removed self-endorser preference to prevent hot-spots.
				Endorser selection is randomized when heights are equal to evenly
				distribute endorsement load across peers.
			*/
			// prefer host peer
