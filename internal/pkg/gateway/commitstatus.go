...
func (gs *Server) CommitStatus(ctx context.Context, signedRequest *gp.SignedCommitStatusRequest) (*gp.CommitStatusResponse, error) {
	...
	/*
		DRUNIX :-
		Lite peers do not commit transactions to the ledger, so they cannot fetch
		the commit status directly. To handle this, CommitStatus() requests are
		forwarded to committing peers that maintain the ledger state.
	*/
	if gs.committingPeerClient != nil {
		return gs.committingPeerClient.CommitStatus(ctx, signedRequest)
	}
