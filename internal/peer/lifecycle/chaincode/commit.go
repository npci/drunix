...
import (
	...
	"github.com/npci/drunix/internal/peer/chaincode"
...
func (c *Committer) Commit() error {
	...
	*/
	schema, err := ReadSqlFile(c.Input.SqlSchema)
	if err != nil {
		return err
	}
	...
	/*
		DRUNIX:
		Added updated DB schema to the envelop
	*/
	env.SchemaUpdate = schema
...
func ReadSqlFile(path string) (string, error) {
	/*
		DRUNIX:
		The path consists of a ".sql" file which is used to upgrade the DB and tables.
	*/
	...
	if err != nil {
		fmt.Printf("Error reading sql-config file: path:%s\n, err: %v\n", path, err)
		return "", err
	}
