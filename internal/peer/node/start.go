...
import (
	...
	"github.com/npci/drunix/core/aclmgmt"
	"github.com/npci/drunix/core/cclifecycle"
	"github.com/npci/drunix/core/chaincode"
	"github.com/npci/drunix/core/chaincode/accesscontrol"
	...
	"github.com/npci/drunix/core/dispatcher"
	"github.com/npci/drunix/core/endorser"
	authHandler "github.com/npci/drunix/core/handlers/auth"
	endorsement2 "github.com/npci/drunix/core/handlers/endorsement/api"
	endorsement3 "github.com/npci/drunix/core/handlers/endorsement/api/identities"
	...
	"github.com/npci/drunix/core/ledger/cceventmgmt"
	"github.com/npci/drunix/core/ledger/kvledger"
	"github.com/npci/drunix/core/ledger/ledgermgmt"
	"github.com/npci/drunix/core/ledger/snapshotgrpc"
	...
	"github.com/npci/drunix/core/scc/cscc"
	"github.com/npci/drunix/core/scc/lscc"
	"github.com/npci/drunix/core/scc/qscc"
	"github.com/npci/drunix/core/transientstore"
	"github.com/npci/drunix/discovery"
	"github.com/npci/drunix/discovery/endorsement"
	discsupport "github.com/npci/drunix/discovery/support"
	discacl "github.com/npci/drunix/discovery/support/acl"
	ccsupport "github.com/npci/drunix/discovery/support/chaincode"
	"github.com/npci/drunix/discovery/support/config"
	"github.com/npci/drunix/discovery/support/gossip"
	gossipcommon "github.com/npci/drunix/gossip/common"
	gossipgossip "github.com/npci/drunix/gossip/gossip"
	gossipmetrics "github.com/npci/drunix/gossip/metrics"
	gossipprivdata "github.com/npci/drunix/gossip/privdata"
	gossipservice "github.com/npci/drunix/gossip/service"
	peergossip "github.com/npci/drunix/internal/peer/gossip"
	"github.com/npci/drunix/internal/peer/version"
	"github.com/npci/drunix/internal/pkg/comm"
	"github.com/npci/drunix/internal/pkg/gateway"
	"github.com/npci/drunix/internal/pkg/gateway/event"
...
var nodeStartCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		return acquireDistributedLockAndServe(args)
	},
}
...
func serve(args []string) error {
	...
	// DRUNIX: if KVStore enabled initialize KVStore transient store
	var kvTransientStoreProvider transientstore.StoreProvider
	if viper.GetBool("peer.kvstore.enabled") {
		kvTransientStoreProvider, err = transientstore.NewKVStoreProvider(metricsProvider)
		if err != nil {
			return errors.WithMessage(err, "failed to open kv transient store")
		}
	}

	// consider peer address as the default peer endpoint
	peerEndpoint := coreConfig.PeerAddress
	// if neither external nor gossip endpoint is set, peer endpoint will be the default peer address

	if externalEndpoint := viper.GetString("peer.gossip.externalEndpoint"); externalEndpoint != "" {
		// if external endpoint is set, use it as the peer endpoint
		peerEndpoint = externalEndpoint
	} else if internalEndpoint := viper.GetString("peer.gossip.endpoint"); internalEndpoint != "" {
		// if external endpoint is not set, but gossip endpoint is set, use it as the peer endpoint
		peerEndpoint = internalEndpoint
	}

	chaincodeEventPublisherConfig := publisher.LoadConfig()
	chaincodeEventPublisherConfig.PeerEndpoint = peerEndpoint
	chaincodeEventPublisher, err := event.NewPublisher(chaincodeEventPublisherConfig)
	if err != nil {
		logger.Panicf("Failed to create chaincode event publisher for config %#v: %s", chaincodeEventPublisherConfig, err)
	}
	...
	peerInstance := &peer.Peer{
		...
		KVStoreProvider:          kvTransientStoreProvider,
		CryptoProvider:           factory.GetDefault(),
		OrdererEndpointOverrides: deliverServiceConfig.OrdererEndpointOverrides,
		ChaincodeEventPublisher:  chaincodeEventPublisher,
		LitePeerEnabled:          coreConfig.LitePeerEnabled,
		CommittingPeerConfig:     coreConfig.CommittingPeerConfig,
	...
	peerInstance.Initialize(
		...
		plugin.MapBasedMapperAdapter(validationPluginsByNameAdapter),
	)
	...
	if coreConfig.GatewayOptions.Enabled {
		if coreConfig.DiscoveryEnabled {
			...
			gatewayServer := gateway.CreateServer(
				...
				builtinSCCs,
				coreConfig.CommittingPeerConfig,
			)
