...
func ChaincodeInvokeOrQuery(
	...
	if invoke {
		if proposalResp != nil {
			...
			/*
				DRUNIX
				CLI INVOKE FIX - CLI INVOKE was going through vanilla flow earlier.
				Populating the LeanEnv field before sending to the orderer.
			*/
			env.LeanEnv, err = protoutil.GetLeanEnvFromCommonEnv(env)
			if err != nil {
				return proposalResp, errors.WithMessagef(err, "error while converting vanilla to lean format sending transaction for %s", funcName)
			}

			env.Type = pcommon.HeaderType_ENDORSER_TRANSACTION
