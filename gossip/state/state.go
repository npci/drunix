...
import (
	...
	common2 "github.com/npci/drunix/gossip/common"
	...
	for {
		...
		case <-s.payloads.Ready():
			...
			for payload := s.payloads.Pop(); payload != nil; payload = s.payloads.Pop() {
				...
				s.logger.Infof(util.ColorGreen("[%s] Total time taken to process block %d with transaction(s) %d is: %s"), s.chainID, rawBlock.Header.Number, len(rawBlock.Data.Data), time.Since(elapsedTime))
			}
		case <-s.stopCh:
