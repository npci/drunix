...
import (
	...
	"github.com/npci/drunix/gossip/gossip"
	...
	"github.com/npci/drunix/gossip/state"
	"github.com/npci/drunix/gossip/util"
	corecomm "github.com/npci/drunix/internal/pkg/comm"
	"github.com/npci/drunix/internal/pkg/identity"
	"github.com/npci/drunix/internal/pkg/peer/blocksprovider"
...
func (g *GossipService) InitializeChannel(channelID string, ordererSource *orderers.ConnectionSource, store *transientstore.Store, support Support) {
	...
	dataRetriever := gossipprivdata.NewDataRetriever(channelID, store, support.Committer, g.serviceConfig.LitePeerEnabled)
	...
	fetcher := gossipprivdata.NewPuller(g.metrics.PrivdataMetrics, support.CollectionStore, g.gossipSvc, dataRetriever,
		collectionAccessFactory, channelID, g.serviceConfig.BtlPullMargin, g.privdataConfig.DisableGetPrivateDataRequest)
	...
	coordinatorConfig := gossipprivdata.CoordinatorConfig{
		...
		LitePeerLevelDBTransientStorePurgeInterval: g.serviceConfig.LitePeerLevelDBTransientStorePurgeInterval,
	...
	if g.deliveryService[channelID] != nil {
		...
		/*
			DRUNIX
			When lite peer mode is enabled, set leader election and static org leader to false
			to prevent starting the delivery service for block retrieval from orderers.
		*/
		if g.serviceConfig.LitePeerEnabled {
			leaderElection = false
			isStaticOrgLeader = false
		}
