...
import (
	...
	common_utils "github.com/npci/drunix/common/util"
	"github.com/npci/drunix/gossip/api"
	"github.com/npci/drunix/gossip/comm"
	"github.com/npci/drunix/gossip/common"
	"github.com/npci/drunix/gossip/discovery"
	"github.com/npci/drunix/gossip/election"
	"github.com/npci/drunix/gossip/filter"
	"github.com/npci/drunix/gossip/gossip/algo"
	"github.com/npci/drunix/gossip/gossip/msgstore"
	"github.com/npci/drunix/gossip/gossip/pull"
	"github.com/npci/drunix/gossip/metrics"
	"github.com/npci/drunix/gossip/protoext"
...
func NewGossipChannel(pkiID common.PKIidType, org api.OrgIdentityType, mcs api.MessageCryptoService,
	...
	litePeerEnabled bool, committingPeerID string) GossipChannel {
	gc := &gossipChannel{
		...
		committingPeerID:          committingPeerID,
	}
...
func (gc *gossipChannel) requestStateInfo() {
	...
	/*
	   DRUNIX:- Lite peers depend on committing peers for authoritative ledger height.
	   Adding the committing peer to endpoints ensures stateInfo propagation and
	   correct height awareness.
	*/
	if gc.litePeerEnabled {
		endpoints = gc.addCommittingPeerToEndpoints(endpoints)
	}
...
func (gc *gossipChannel) HandleMessage(msg protoext.ReceivedMessage) {
	...
	if protoext.IsStateInfoPullRequestMsg(m.GossipMessage) {
		...
		/*
		   DRUNIX:- To support lite peer synchronisation, committing peers explicitly
		   respond to same-org StateInfoPullRequestMsg with StateInfoUpdate, allowing
		   lite peers to learn the current authoritative ledger height.
		*/
		if !gc.litePeerEnabled && bytes.Equal(gc.selfOrg, orgID) {
			stateInfoMessage, err := gc.setupSignedStateInfoMessage()
			if err != nil {
				gc.logger.Warningf("failed to respond with stateInfo message, err:, %v", err)
			} else {
				gc.Send(stateInfoMessage, &comm.RemotePeer{Endpoint: msg.GetConnectionInfo().Endpoint, PKIID: msg.GetConnectionInfo().ID})
			}
		}
		return
	}
	...
	if protoext.IsDataMsg(m.GossipMessage) || protoext.IsStateInfoMsg(m.GossipMessage) {
		...
		if protoext.IsDataMsg(m.GossipMessage) {
			...
			/*
				DRUNIX:-
				Lite peers now advertise the committing peer’s ledger height upon receiving a stateInfoMsg.
				This prevents endorsements on stale data by ensuring requests are routed to lite peers
				associated with the highest committing peer height.
			*/
			if gc.litePeerEnabled {
				msgPeerID, err := gc.GetIdentityByPKIID(msg.GetGossipMessage().GetStateInfo().GetPkiId()).GetCommonName()
				if err != nil {
					gc.logger.Warningf("failed to gossip litepeer height, err: %v", err)
				} else if gc.committingPeerID == msgPeerID {
					gc.logger.Debugf("Gossiping received committing peer height [%v]", msg.GetGossipMessage().GetStateInfo().GetProperties().GetLedgerHeight())
					gc.UpdateLedgerHeight(msg.GetGossipMessage().GetStateInfo().GetProperties().GetLedgerHeight())
				}
			}
...
correct height awareness.
*/
func (gc *gossipChannel) addCommittingPeerToEndpoints(endpoints []*comm.RemotePeer) []*comm.RemotePeer {
	for _, peer := range gc.GetMembership() {
		if committingPeerID, err := gc.GetIdentityByPKIID(peer.PKIid).GetCommonName(); err != nil {
			gc.logger.Warningf("failed to request committing peer stateInfo, err: %v", err)
		} else if committingPeerID == gc.committingPeerID {
			endpoints = append(endpoints, &comm.RemotePeer{Endpoint: peer.PreferredEndpoint(), PKIID: peer.PKIid})
			return endpoints
		}
	}
	return endpoints
