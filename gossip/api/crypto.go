...
// DRUNIX:- GetCommonName extracts the common name from the identity and returns the common name as string
func (pit PeerIdentityType) GetCommonName() (string, error) {
	base64Representation := base64.StdEncoding.EncodeToString(pit)
	sID := &msp.SerializedIdentity{}
	err := proto.Unmarshal(pit, sID)
	if err != nil {
		return "", fmt.Errorf("non SerializedIdentity: %s", base64Representation)
	}

	bl, _ := pem.Decode(sID.IdBytes)
	if bl == nil {
		return "", fmt.Errorf("non PEM encoded identity: %s", base64Representation)
	}

	cert, _ := x509.ParseCertificate(bl.Bytes)
	if cert == nil {
		return "", fmt.Errorf("non x509 identity: %s", base64Representation)
	}
