...
type puller struct {
	...
	disableGetPrivateDataRequest bool
}
...
func NewPuller(metrics *metrics.PrivdataMetrics, cs privdata.CollectionStore, g gossip,
	dataRetriever PrivateDataRetriever, factory CollectionAccessFactory, channel string, btlPullMargin uint64, disableGetPrivateDataRequest bool) *puller {
	p := &puller{
		...
		disableGetPrivateDataRequest: disableGetPrivateDataRequest,
	}
...
func (p *puller) createResponse(message protoext.ReceivedMessage) []*protosgossip.PvtDataElement {
	...
	p.logger.Infof("Got private data request from %s", message.GetConnectionInfo().String())
	if p.disableGetPrivateDataRequest {
		p.logger.Infof("Returned empty response for the private data request from %s", message.GetConnectionInfo().String())
		return returned
	}
	...
	p.logger.Infof("Returning response for the private data request from %s", message.GetConnectionInfo().String())
	return returned
}
