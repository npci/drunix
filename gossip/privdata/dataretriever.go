...
func (dr *dataRetriever) CollectionRWSet(digests []*protosgossip.PvtDataDigest, blockNum uint64) (Dig2PvtRWSetWithConfig, bool, error) {
	...
	if dr.store.KVStore != nil {
		if !dr.litePeerEnabled {
			return dr.CollectionRWSetFromKVStore(digests, blockNum)
		}

		result, fromLedger, err := dr.CollectionRWSetFromKVStore(digests, blockNum)
		if err == nil && result != nil && (len(result) > 0) {
			return result, fromLedger, err
		}

		// dr.logger.Errorf("Error in retriving the Pvt data from KVStore. err:%+v", err)
