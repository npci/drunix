...
type CoordinatorConfig struct {
	...
	LitePeerLevelDBTransientStorePurgeInterval time.Duration
...
func customPurge(logger util.Logger, interval time.Duration, transientBlockRetention uint64, transientStore *transientstore.Store) {
	...
	for range ticker.C {
		...
		if transientStore.KVStore == nil {
			logger.Errorf("KVStore cannot be nil for lite Peer")
			return
		}
		...
		if blockNum > transientBlockRetention {
			err := transientStore.PurgeBelowHeight(blockNum - transientBlockRetention)
			if err != nil {
				logger.Errorf("Failed purging data from transient store at block [%d]: %s", blockNum, err)
			}
		}
...
func (c *coordinator) StoreBlock(block *common.Block, privateDataSets util.PvtDataCollections) error {
	...
	if c.CapabilityProvider.Capabilities().LeanFormatEnabled() && !protoutil.IsConfigBlock(block) {
		pvtdataToRetrieve, err := c.getLtxPvtdataInfoFromBlock(block)
		if err != nil {
			c.logger.Warningf("Failed to get private data info from block: %s", err)
			return err
		}

		retrievedLtxPvtdata, err := pdp.RetrieveLtxPvtdata(pvtdataToRetrieve)
		if err != nil {
			c.logger.Warningf("Failed to retrieve ltx pvtdata: %s", err)
			return err
		}

		blockAndPvtData.PvtData = retrievedLtxPvtdata.blockPvtdata.PvtData
		blockAndPvtData.MissingPvtData = retrievedLtxPvtdata.blockPvtdata.MissingPvtData

		commitStart := time.Now()
		err = c.CommitLegacyLtx(blockAndPvtData, &ledger.CommitOptions{})
		c.reportCommitDuration(time.Since(commitStart))
		if err != nil {
			return errors.Wrap(err, "commit failed")
		}

		if retrievedLtxPvtdata != nil && !c.store.LitePeerEnabled {
			go retrievedLtxPvtdata.Purge()
		}
	...
	pvtDataFetchStartTime := time.Now()
	...
	logger.Infof("[%s] Pvt Data Fetch for block [%d] with %d transactions in %dms", c.ChainID, block.Header.Number, len(block.Data.Data), time.Since(pvtDataFetchStartTime).Milliseconds())
	...
	if retrievedPvtdata != nil && !c.store.LitePeerEnabled {
