...
import (
	...
	util2 "github.com/npci/drunix/common/util"
	...
	privdatamocks "github.com/npci/drunix/gossip/privdata/mocks"
...
// DRUNIX : Fetcher interface implementation
func (f *fetcherMock) ltxFetch(dig2src ltxDig2sources) (*privdatacommon.FetchedPvtDataContainer, error) {
	uniqueEndorsements := make(map[string]interface{})
	for _, endorsements := range dig2src {
		for _, endorsement := range endorsements {
			...
			if !exists {
				f.t.Fatalf("Encountered a non-expected endorser: %s", string(endorserbytes))
			}
			uniqueEndorsements[string(endorserbytes)] = struct{}{}
		}
	}
	require.True(f.t, digests(f.expectedDigests).Equal(digests(dig2src.keys())))
	require.Equal(f.t, len(f.expectedEndorsers), len(uniqueEndorsements))
