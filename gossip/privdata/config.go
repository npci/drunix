...
type PrivdataConfig struct {
	...
	/*
		Drunix::
	*/
	// PrivateDataDissemination specifies the pvtData dissemination  policy for the peer
	PrivateDataDissemination bool
	// DisableGetPrivateDataRequest is a flag that indicates whether private data should be sent as gossip GetPrivateReq response.
	// Defaults to false, which sents the private data
...
func (c *PrivdataConfig) loadPrivDataConfig() {
	...
	c.PrivateDataDissemination = true
	if viper.IsSet("peer.gossip.pvtData.privateDataDissemination") {
		c.PrivateDataDissemination = viper.GetBool("peer.gossip.pvtData.privateDataDissemination")
	}
	...
	c.DisableGetPrivateDataRequest = viper.GetBool("peer.gossip.pvtData.disableGetPrivateDataRequest")
}
