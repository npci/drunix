...
PROJECT_VERSION=$(DRUNIX_VER)-snapshot-$(EXTRA_VERSION)
...
TWO_DIGIT_VERSION = $(shell echo $(DRUNIX_VER) | sed -e  's/^v\(.*\)/\1/' | cut -d '.' -f 1,2)

PKGNAME = github.com/npci/drunix
ARCH=$(shell go env GOARCH)
MARCH=$(shell go env GOOS)-$(shell go env GOARCH)

# defined in common/metadata/metadata.go
METADATA_VAR = Version=$(DRUNIX_VER)
...
RELEASE_IMAGES = baseos ccenv orderer peer tools vscc
RELEASE_PLATFORMS = darwin-amd64 darwin-arm64 linux-amd64 linux-arm64 windows-amd64
TOOLS_EXES = configtxgen configtxlator cryptogen discover ledgerutil osnadmin peer vscc
...
docker-thirdparty: docker-thirdparty-couchdb
	...
	docker pull npcioss/drunix-ccenv:1.4
...
$(BUILD_DIR)/images/vscc/$(DUMMY):    BUILD_ARGS=--build-arg GO_TAGS=${GO_TAGS}

$(BUILD_DIR)/images/%/$(DUMMY):
	@echo "Building Docker image $(DOCKER_NS)/drunix-$*"
	@mkdir -p $(@D)
	$(DBUILD) -f images/$*/Dockerfile \
		--build-arg GO_VER=$(GO_VER) \
		--build-arg UBUNTU_VER=$(UBUNTU_VER) \
		--build-arg DRUNIX_VER=$(DRUNIX_VER) \
		--build-arg TARGETARCH=$(ARCH) \
		--build-arg TARGETOS=linux \
		$(BUILD_ARGS) \
		-t $(DOCKER_NS)/drunix-$* ./$(BUILD_CONTEXT)
	@touch $@
...
dist/%: release/% ccaasbuilder
	...
	cd release/$(@F) && tar -czvf npcioss-drunix-$(@F).$(PROJECT_VERSION).tar.gz *

.PHONY: docker-list
docker-list: $(RELEASE_IMAGES:%=%-docker-list)
%-docker-list:
	@echo $(DOCKER_NS)/drunix-$*:$(DOCKER_TAG)

.PHONY: docker-clean
docker-clean: $(RELEASE_IMAGES:%=%-docker-clean)
%-docker-clean:
	-@for image in "$$(docker images --quiet --filter=reference='$(DOCKER_NS)/drunix-$*:$(DOCKER_TAG)')"; do \
...
dist-clean:
	-@for platform in $(RELEASE_PLATFORMS) ""; do \
		[ -z "$$platform" ] || rm -rf release/$${platform}/npcioss-drunix-$${platform}.$(PROJECT_VERSION).tar.gz; \
	done
